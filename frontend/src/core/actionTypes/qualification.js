const prefix = "qualification";

//GET
export const FETCH_QUALIFICATION_BY_ID = `${prefix}/FETCH_QUALIFICATION_BY_ID`;
export const SET_QUALIFICATION = `${prefix}/SET_QUALIFICATION`;

//POST
export const SAVE_QUALIFICATION = `${prefix}/SAVE_QUALIFICATION`;
export const SAVE_QUALIFICATION_LOADING = `${prefix}/SAVE_QUALIFICATION_LOADING`;
export const SAVE_QUALIFICATION_SUCCESS = `${prefix}/SAVE_QUALIFICATION_SUCCESS`;
export const SAVE_QUALIFICATION_FAILED = `${prefix}/SAVE_QUALIFICATION_FAILED`;

//PUT
export const UPDATE_QUALIFICATION = `${prefix}/UPDATE_QUALIFICATION`;
export const UPDATE_QUALIFICATION_SUCCESS = `${prefix}/UPDATE_QUALIFICATION_SUCCESS`;
export const UPDATE_QUALIFICATION_FAILED = `${prefix}/UPDATE_QUALIFICATION_FAILED`;

//DELETE
export const DELETE_QUALIFICATION = `${prefix}/DELETE_QUALIFICATION`;
export const DELETE_QUALIFICATION_SUCCESS = `${prefix}/DELETE_QUALIFICATION_SUCCESS`;
export const DELETE_QUALIFICATION_FAILED = `${prefix}/DELETE_QUALIFICATION_FAILED`;